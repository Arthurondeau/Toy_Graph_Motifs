...
def create_radial_graph(radial=5, length=50, multidigraph=False):
...
def create_concentric_graph_old(
    ...
    for i in range(zones):
        for j in range(startnum, startnum + radial):
            ...
            if straight_edges:
                ...
                geom = create_curved_linestring_old(fc, sc, radius * (i + 1), offset=offset)
            G.add_edge(
                fn,
                sn,
                geometry=geom,
...
def create_concentric_graph(
    radial=8, zones=3, radius=30, straight_edges=True, center=True, multidigraph=False
):
    """Create a concentric graph, where nodes are on circular zones, connected to their nearest neighbors and to the next zone.
...
def create_curved_linestring(startpoint, endpoint, radius, offset=0):
    ...
    link = shapely.LineString([coords[-1], endpoint])
    arc = shapely.LineString(coords)
    return shapely.union_all([arc,link])

# Simpler but less general function in the meantime
def create_curved_linestring_old(startpoint, endpoint, radius, offset=0):
    """Create a curved linestring between the two selected points.
